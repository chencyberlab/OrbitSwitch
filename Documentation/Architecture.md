...
Peek deliberately ignores `currentSpaceOnly`, `includeMinimized`, and `includeHiddenApps`: the point of hovering a Dock icon is to reach any window that application has, including one parked on another Desktop. The minimum-size and excluded-bundle filters still apply, so a window the user deliberately filtered out stays out. Closing a window from a peek uses the same rule as the switcher — `AXPress` returning success means the request was delivered, not accepted, so the card survives until Core Graphics confirms the window ID is gone.
